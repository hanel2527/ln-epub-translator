...
def start_translate(
    ...
    print(f"Start translating {source_path.name}")
    ...
    def on_progress(chapter_idx: int, total: int, title: str) -> None:
        progress((chapter_idx + 1) / total, desc=f"Ch {chapter_idx+1}/{total}: {title}")
    ...
    try:
        ...
        return msg, str(result.output_path), str(result.clean_path)
